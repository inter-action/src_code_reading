...
func log(color string, info interface{}) {
	...
	fmt.Printf("%s: %s%s%s\n", outputType, color, info, "\x1b[0m") //console 中输出颜色的格式
}
	...
	for path, _ := range buildMap.Watch {
		...
		if matchPath, err := filepath.Glob(path); err == nil {
			for _, path := range matchPath {
				...
				if _, ok := watchDir[dirPath]; !ok { //watchDir 用来过滤是否watch了该文件
...
func runCMD(command string, daemon bool) error {
	...
	if runtime.GOOS == "windows" {
		...
		shell = "/bin/sh" // http://superuser.com/questions/97614/what-exactly-is-the-sh-command
		flag = "-c"       // 后面接命令
	}
...
func main() {
	...
	app.Action = func(c *cli.Context) {
		...
		done := make(chan bool) // todo: 这个地方是什么原理，需要看下 chan
		...
		if len(buildMap.Watch) != 0 {
			<-done // 由于go 的 channel 永远会在接受端等待数据的时候block住, 这样的写法就会一直block,从而阻止main go routine 的退出
		}
	}
	app.Run(os.Args)
}
